...
def mcmc(data,            uncert=None,      func=None,      indparams=[],
  ...
  if walk == "snooker":
    ...
    for i in range(nfree):
      ...
      Z[:hsize, :, ind] = np.random.uniform(pmin[ind], pmax[ind],
                                         (hsize, nchains)        )
    # Evaluate models for initial samples of Z
    if mpi:
  ...
  for i in np.arange(chainsize):
    ...
    if mpi:
      ...
      iout = np.where(~outflag)[0]
      if len(iout) > 0:
        fargs = [nextp[iout, 0:mpars]] + indparams  # List of function's arguments
        models[iout] = func(*fargs)
    ...
    if ((i+1) % intsteps == 0) and (i > 0):
      ...
      mu.msg(1, "Out-of-bound Trials:\n {:s}".
                 format(str(np.sum(outbounds, axis=0))), log)
